...
module.exports = {
  ...
  getMessages: function() {
    ...
    _.each(sequenceQueue, function (codeId) {
      ...
      _.each(groupList, function (groupId) {
        ...
        messageRows.push(prefix + message);
        _.each(group.locations, function (location) {
          messageRows.push('  > ' + location + '\n');
        });
      });
    });
  ...
  print: function() {
    var messageRows = this.getMessages();

    messageRows.forEach(function(msg) { process.stdout.write(msg); });
  },
  ...
  writeFile: function (dir, file) {
    ...
    fs.writeFileSync(path.join(dir, file), messageRows.join(''));
  }
};
